...
const ALLOWED_EXTS = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'mp4', 'avi', 'mkv', 'mov', '3gp', 'flv', 'wmv']);
...
function sticker6(img, url) {
    return new Promise(async (resolve, reject) => {
        ...
        if (!ALLOWED_EXTS.has(type.ext)) return reject(new Error(`Unsupported file type: ${type.ext}`));
        const safeExt = type.ext.replace(/[^a-zA-Z0-9]/g, '');
        const tmp = path.join(__dirname, '../tmp', `${+new Date()}.${safeExt}`);
        const out = path.join(__dirname, '../tmp', `${+new Date()}.${safeExt}.webp`);
