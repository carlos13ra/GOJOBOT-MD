...
if (!("primaryLastSeen" in chat)) chat.primaryLastSeen = 0
...
if (nuevo && typeof nuevo === "string" && nuevo.trim() && nuevo!== actual) {
user.name = nuevo
}} catch {}
...
if (opts["que"] && m.text &&!(isPrems)) {
const queque = this.msgque, time = 1000 * 5
const previousID = queque[que.length - 1]
que.push(m.id || m.key.id)
...
// Primary Bot Logic - FIXED
const primary = global.db.data.chats[m.chat].primaryBot
const now = Date.now()

if (primary && primary!== this.user.jid) {
  const primaryBotConn = global.conns.find(c =>
    c.user?.jid === primary && c.ws?.socket?.readyState === ws.OPEN
  )
  const participants = m.isGroup
   ? (await this.groupMetadata(m.chat).catch(() => ({ participants: [] }))).participants
    : []
  const primaryBotInGroup = participants.some(p => p.jid === primary)

  if (primaryBotConn && primaryBotInGroup) {
    // Primary está vivo, actualiza lastSeen y bloquea a los demás
    global.db.data.chats[m.chat].primaryLastSeen = now
    throw!1
  }

  // Timeout de 60s antes de liberar el chat
  const lastSeen = global.db.data.chats[m.chat].primaryLastSeen || 0
  if (now - lastSeen <= 60000) {
    // Aún esperamos al primary
    throw!1
  }
  // Si pasa de 60s, deja pasar a los demás bots
}
...
global.dfail = (type, m, conn) => {
 const msg = {
   ...
   owner: ` ׄ 🌾 ׅ 𝙀𝙡 𝙘𝙤𝙢𝙖𝙣𝙙𝙤 *${global.comando}* 𝙨𝙤𝙡𝙤 𝙥𝙪𝙚𝙙𝙚 𝙨𝙚𝙧 𝙪𝙨𝙖𝙙𝙤 𝙥𝙤𝙧 𝙡𝙤𝙨 𝙙𝙚𝙨𝙖𝙧𝙤𝙡𝙖𝙙𝙤𝙧𝙚𝙨 𝙙𝙚𝙡 𝙗𝙤𝙩.`,
   mods: ` ׄ 🍉 ׅ 𝙀𝙡 𝙘𝙤𝙢𝙖𝙣𝙙𝙤 *${global.comando}* 𝙨𝙤𝙡𝙤 𝙥𝙪𝙚𝙙𝙚 𝙨𝙚𝙧 𝙪𝙨𝙖𝙙𝙤 𝙥𝙤𝙧 𝙡𝙤𝙨 𝙢𝙤𝙙𝙚𝙧𝙖𝙙𝙤𝙧𝙚𝙨 𝙙𝙚𝙡 𝙗𝙤𝙩.`,
   ...
   botAdmin: ` ׄ 🚀 ׅ 𝙋𝙖𝙧𝙖 𝙚𝙟𝙚𝙘𝙪𝙩𝙖𝙧 𝙚𝙡 𝙘𝙤𝙢𝙖𝙣𝙙𝙤 *${global.comando}* 𝙙𝙚𝙗𝙤 𝙨𝙚𝙧 𝙖𝙙𝙢𝙞𝙣𝙞𝙨𝙩𝙧𝙖𝙙𝙤𝙧 𝙙𝙚𝙡 𝙜𝙧𝙪𝙥𝙤.`,
