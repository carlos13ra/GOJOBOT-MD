...
const handler = async (m, { conn, text, command }) => {
  try {
    ...
    const isAudio =
      ["ytaudio", "playaudio"].includes(command)
    ...
    if (isAudio) {
      ...
      await conn.sendMessage(
        m.chat,
        {
          ...
          fileName: `${sanitize(data.title)}.mp3`
        },
        { quoted: m }
      )
      ...
      await conn.sendMessage(
        m.chat,
        {
          ...
          mimetype: "video/mp4",
          fileName: `${sanitize(data.title)}.mp4`,
          caption: data.title
        },
        { quoted: m }
      )
...
async function getBuffer(url) {
  ...
  if (!res.ok) {
    throw new Error("Error thumbnail")
  }
...
async function tunexaDown(
  ...
  return {
    ...
    title:
      result.title ||
      opts2.prefetched_title,
    duration:
      result.duration ||
      opts2.prefetched_duration,
    thumbnail:
      result.thumbnail ||
      opts2.prefetched_thumbnail,
    download_url:
      result.download_url,
    format:
      result.audio_format ||
