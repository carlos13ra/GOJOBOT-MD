...
async function getFacebookMedia(url) {
  ...
  try {
    const { stdout } = await execPromise(`ffprobe -v error -show_entries format=duration,size -of default=noprint_wrappers=1:nokey=1 "${target.videoUrl}"`)
    const lines = stdout.trim().split('\n')
    segundos = parseFloat(lines[0]) || 0
    if (lines[1] &&!bytes) bytes = parseInt(lines[1]) || 0
  } catch {
    try {
      const res = await fetch(target.videoUrl, { headers: { 'Range': 'bytes=0-1048576' } })
 ...
 ? `${horas}:${min.toString().padStart(2,'0')}:${seg.toString().padStart(2,'0')}`
  ...
  const sizeMB = bytes? (bytes / 1024 / 1024).toFixed(2) + ' MB' : null
  ...
  return {
    ...
    url: target.videoUrl,
    thumbnail: null,
    duration: segundos? duracion : null,
    sizeMB
  }
}
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text.match(/facebook\.com|fb\.watch|video\.fb\.com/)) {
    return m.reply('🥢 El enlace es invalido, envía un link de Facebook válido')
  }
  try {
    const data = await getFacebookMedia(text)
    if (!data) return m.reply('🍜 No se pudo obtener el contenido.')
    const caption = ` \`﹒⌗﹒🥢 .ৎ˚₊‧  𝖿ᥲᥴᥱᑲ᥆᥆k D᥆ᥕᥒᥣ᥆ᥲძᥱr\`

${data.title? `˙.꒷✿︎.𖦹˙ 𝗍í𝗍ᥙᥣ᥆ › *${data.title}*` : ''}
${data.resolution? `˙.꒷ꕥ.𖦹˙ rᥱs᥆ᥣᥙᥴіóᥒ › *${data.resolution}*` : ''}
${data.format? `˙.꒷❍.𖦹˙ 𝖿᥆rmᥲ𝗍᥆ › *${data.format}*` : ''}
${data.duration? `˙.꒷ⴵ.𖦹˙ ძᥙrᥲᥴіóᥒ › *${data.duration}*` : ''}
${data.sizeMB? `˙.꒷⎙.𖦹˙ 𝗍ᥲmᥲñ᥆ › *${data.sizeMB}*` : ''}
˙.꒷🜸.𖦹˙ ᥱᥒᥣᥲᥴᥱ › \`\`\`${text}\`\`\`

> ${dev}`
    if (data.type === 'video') {
      await conn.sendFile(m.chat, data.url, 'Miku_sakura.mp4', caption, m) 
