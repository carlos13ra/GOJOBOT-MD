...
try {
  ...
  let name = await (async () => global.db.data.users[userId].name || (async () => { 
      try { 
  ...
  // 🔹 FOTO DE PERFIL: directo, si es privada manda imagen genérica o vacía
  let ppBuffer
  try {
    ppBuffer = await conn.profilePicture(userId, 'image')
  } catch {
    ppBuffer = Buffer.from('') // si quieres, puedes usar fs.readFileSync('./default.jpg')
  }

  const currency = global.currency || 'Coins'

  // ✨ ESTILO BONITO CON FONTS
  const text = `╔═══〔 𝙋𝙀𝙍𝙁𝙄𝙇 〕═══╗
👤 ${name}
✎ ${description}
╚═══════════════╝
...
│ ${progreso}
│ Ranking: #${rank}
│ Premium: ${premium ? isLeft : 'No'}
└────────────

┌─〔 𝙀𝘾𝙊𝙉𝙊𝙈𝙄𝘼 〕
│ Cartera: ${coin.toLocaleString()}
...
│ Valor: ${haremValue.toLocaleString()}${favLine}
└────────────
...
└────────────`
  ...
  await conn.sendMessage(
    m.chat,
    { image: ppBuffer, caption: text, mentions: [userId] },
    { quoted: fkontak }
  )
